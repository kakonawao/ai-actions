...
def _get_pr_review_comments(pr_number: str, repo: str) -> str:
    """Fetches all review comments from a pull request using GitHub's REST API."""
    print("[INFO] Fetching PR review comments...")
    try:
        # Use gh api to call the REST endpoint for pull request review comments
        ...
        if not comments_data:
            print("[INFO] No review comments found.")
            return "No review comments."

        formatted_comments = []
        for comment in comments_data:
            path = comment.get("path")
            line = comment.get("line") or comment.get("original_line") # 'line' for new code, 'original_line' for old code
        ...
        print(f"[ERROR] Failed to fetch or parse PR review comments: {e}")
        return "Could not fetch PR review comments."
...
def run_agent(user_prompt: str):
    ...
    llm = ChatGoogleGenerativeAI(model=gemini_model_name, temperature=0, rate_limiter=rate_limiter)
    ...
    agent = create_agent(model=llm, tools=tools, system_prompt=system_prompt_content)
...
def main():
    ...
    if agent_mode == "draft":
        ...
        if not pr_number or not repo:
            print("[ERROR] PR_NUMBER or GITHUB_REPOSITORY not set for revise mode.")
            return
            
        pr_diff = subprocess.check_output(['gh', 'pr', 'diff', pr_number]).decode('utf-8')
        review_comments = _get_pr_review_comments(pr_number, repo) # Renamed function and variable
        ...
        Start by listing the files in the current directory to understand the project structure.
        Then, read the relevant files to understand the existing code.
        Finally, use the write_file tool to create or update the necessary files with your proposed revisions based on the review comments.
        """
        run_agent(user_prompt)
